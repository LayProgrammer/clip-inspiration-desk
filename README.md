...
首页会先强调产品边界：它不替代剪映，而是帮你从素材里找故事。

![剪辑灵感台首页](./docs/assets/cover.png)

选定一条剪辑路线后，可以生成路线执行包，带着清单去剪映、Premiere 或 CapCut 里照着执行。

![路线执行包](./docs/assets/route-pack.png)
...
- 素材篮控制：
  ...
  - 禁用
- 针对选中路线生成专属执行包。
- 对执行包继续追问，例如压缩到 20 秒、不要露脸、改标题封面、换成更有反差的节奏。
- 复制执行包。
- 导出 Markdown 执行包。
- 本地设置面板：
  - 配置 AI provider
